...
// Same as Lab 3
	DisableInterrupts();
	(*semaPt) = (*semaPt) - 1; // from my design, all threads in the tcb point to an initial global value of 0  
	if((*semaPt) < 0){
...
void OS_FIFO_Init(void){
	// ****IMPLEMENT THIS****
	// Same as Lab 3
	PutI = GetI = 0; // FIFO is empty, so both put and get will point to the top of the FIFO
...
// Same as Lab 3
 if(CurrentSize == FSIZE){
		...
		Fifo[PutI] = data; // Put
		PutI = (PutI+1)%FSIZE; // ensure that PutI is between 0 and FSIZE - 1; so if the oldest data is at the end of buffer, place the newest at the start
...
// 2) no need to unlock PD6
	...
	GPIO_PORTD_DEN_R |= 0x40;// 4) configure PD6 as GPIO
...
// 7) enable digital I/O on PD6  
	...
	NVIC_PRI0_R = (NVIC_PRI0_R & 0x00FFFFFF)|priority;// priority on Port D edge trigger is NVIC_PRI0_R	31 – 29
